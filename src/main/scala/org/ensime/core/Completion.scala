...
import scala.concurrent.backport.{ Future, Await }
...
trait CompletionControl {
          ...
          case Apply(fun, _) => {
            fun match {
              ...
              case Select(qual, name) if qual.pos.isDefined && qual.pos.isRange => {
                val prefix = if (patched) "" else name.decode
  ...
  def fetchTypeSearchCompletions(prefix: String, maxResults: Int): Future[Option[List[CompletionInfo]]] = {
    ...
    import scala.concurrent.backport.ExecutionContext.Implicits.{ global => exe }
