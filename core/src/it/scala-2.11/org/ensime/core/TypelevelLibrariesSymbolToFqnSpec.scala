...
import org.ensime.fixture.{
  EnsimeConfigFixture,
  IsolatedRichPresentationCompilerFixture
}
...
class TypelevelLibrariesSymbolToFqnSpec
    extends EnsimeSpec
  ...
  it should "index all class file in typelevel libraries" in withPresCompiler {
    (config, cc) =>
      val vfs = cc.vfs
        ...
        classes.foreach { scalaClass =>
          ...
          scalaClass.methods.valuesIterator.foreach { methods =>
            methods.foreach { method =>
              val methodSym = cc
                .askSymbolByScalaName(method.scalaName, Some(DeclaredAs.Method))
                .get
